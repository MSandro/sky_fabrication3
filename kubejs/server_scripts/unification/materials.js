...
function findTagUnification(event, tagName) {
    ...
    tagItems.forEach(id => {
        ...
        if (id.toString().split(':')[0] === "minecraft") {
            doUnify = false;
        }
    });
    if (!doUnify) return;
...
onEvent('tags.items', event => {
    ...
    MATERIALS.forEach(material => {
        var double_blacklist = ["bauxite", "chrome", "coal", "diamond", "emerald", "iridium", "lapis", "manganese", "quartz", "ruby", "sulfur", "salt", "carbon", "brass", "zinc"];
        if (!double_blacklist.includes(material)) {
          event.add(`c:${material}_double_ingots`, `modern_industrialization:${material}_double_ingot`);
        }
...
onEvent('recipes', event => {
  ...
  for (let id in MATERIALS) {
    event.remove({input: 'techreborn:' + MATERIALS[id] + '_small_dust', type: 'minecraft:crafting_shapeless'})
